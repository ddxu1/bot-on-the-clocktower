...
// Create new game
fastify.post<{ Reply: { game_id: string } }>(
  "/api/games",
  async (request, reply) => {
    try {
...
// Close nomination
fastify.post<{ Params: { gameId: string } }>(
  "/api/games/:gameId/close-nomination",
  async (request, reply) => {
    try {
      const { gameId } = request.params;
...
fastify.post<{
  ...
  try {
    ...
    const game = await engine.performNightAction(
      gameId,
      playerId,
      role as any,
      targets
