...
export class ClockTowerEngine {
  ...
  async assignRoles(
    ...
    game.updated_at = new Date().toISOString();
    await this.saveGameState(game);
    await this.logAction(gameId, "ASSIGN_ROLES", {
  ...
  async executePlayer(gameId: string, playerId: string): Promise<GameState> {
    ...
    const scarletWoman = game.players.find(
      (p) => p.role === "SCARLET_WOMAN" && p.alive
    );
  ...
  async openNight(gameId: string): Promise<GameState> {
    ...
    await this.saveGameState(game);
    await this.logAction(gameId, "ADVANCE_PHASE", {
      to: "NIGHT",
      day: game.day_number,
    });
    ...
    switch (role) {
      case "POISONER":
        if (!targets || targets.length !== 1)
          throw new Error("Poisoner must target exactly 1 player");
        action = RoleAbilities.poisonerNightAction(game, playerId, targets[0]);
        break;

      case "IMP":
        if (!targets || targets.length !== 1)
          throw new Error("Imp must target exactly 1 player");
        action = RoleAbilities.impNightActionEnhanced(
          game,
          playerId,
          targets[0]
        );
      ...
      case "MONK":
        if (!targets || targets.length !== 1)
          throw new Error("Monk must target exactly 1 player");
        action = RoleAbilities.monkNightAction(game, playerId, targets[0]);
        break;

      case "BUTLER":
        if (!targets || targets.length !== 1)
          throw new Error("Butler must choose exactly 1 master");
        action = RoleAbilities.butlerNightAction(game, playerId, targets[0]);
        break;

      case "FORTUNE_TELLER":
        if (!targets || targets.length !== 2)
          throw new Error("Fortune Teller must target exactly 2 players");
      ...
      case "RAVENKEEPER":
        ...
        if (!targets || targets.length !== 1)
          throw new Error("Ravenkeeper must target exactly 1 player");
  ...
  private async processSetupAbilities(game: GameState): Promise<void> {
    for (const player of game.players) {
      ...
      switch (player.role) {
        case "WASHERWOMAN":
          const washerwomanInfo = RoleAbilities.washerwomanSetup(
            game,
            player.id
          );
        ...
        case "INVESTIGATOR":
          const investigatorInfo = RoleAbilities.investigatorSetup(
            game,
            player.id
          );
      ...
      switch (player.role) {
        ...
        case "UNDERTAKER":
          if (game.day_number > 1) {
            // Undertaker doesn't act first night
            const undertakerInfo = RoleAbilities.undertakerNightAction(
              game,
              player.id
            );
            if (undertakerInfo) {
