...
function getNeighbors(game: GameState, playerId: string): Player[] {
  const alivePlayers = game.players.filter(p => p.alive).sort((a, b) => a.seat - b.seat);
...
function countEvilPairs(game: GameState): number {
  const alivePlayers = game.players.filter(p => p.alive && p.alignment === 'EVIL').sort((a, b) => a.seat - b.seat);
  let pairs = 0;
  ...
  for (let i = 0; i < alivePlayers.length - 1; i++) {
    ...
    const allPlayers = game.players.filter(p => p.alive).sort((a, b) => a.seat - b.seat);
...
export class RoleAbilities {
  ...
  static washerwomanSetup(game: GameState, playerId: string): PlayerInfo {
    const townsFolk: Role[] = ['WASHERWOMAN', 'INVESTIGATOR', 'CHEF', 'EMPATH', 'SLAYER', 'UNDERTAKER'];
    const townsFolkPlayers = game.players.filter(p => p.role && townsFolk.includes(p.role) && p.id !== playerId);
    ...
    const randomPlayers = getRandomPlayers(game.players.filter(p => p.id !== playerId), 2);

    // Ensure one of the two players actually has the role
    const candidates = [targetPlayer, randomPlayers.find(p => p.id !== targetPlayer.id) || randomPlayers[0]];
    ...
    return {
      ...
      information: {
        ...
        message: `One of these players is the ${targetRole}: ${candidates.map(p => p.name).join(', ')}`
      },
      night_received: 0
    };
  }
  ...
  static investigatorSetup(game: GameState, playerId: string): PlayerInfo {
    ...
    const minionPlayers = game.players.filter(p => p.role && minions.includes(p.role));
  ...
  static empathNightAction(game: GameState, playerId: string): PlayerInfo {
    ...
    const evilNeighbors = neighbors.filter(p => p.alignment === 'EVIL').length;
  ...
  static undertakerNightAction(game: GameState, playerId: string): PlayerInfo | null {
    ...
    const executedPlayer = game.players.find(p => p.id === game.last_execution!.player_id);

    return {
      player_id: playerId,
      information_type: 'UNDERTAKER',
      information: {
        executed_player: executedPlayer ? { id: executedPlayer.id, name: executedPlayer.name } : null,
        role: game.last_execution.role,
        message: `${executedPlayer?.name || 'Unknown'} was the ${game.last_execution.role}`
      },
      night_received: game.day_number
    };
  }
  ...
  static slayerDayAction(game: GameState, playerId: string, targetId: string): { success: boolean; message: string } {
  ...
  static poisonerNightAction(game: GameState, playerId: string, targetId: string): NightAction {
    const target = game.players.find(p => p.id === targetId);
  ...
  static impNightAction(game: GameState, playerId: string, targetId: string): NightAction {
    const target = game.players.find(p => p.id === targetId);
    ...
    if (targetId === playerId) {
      const minions = game.players.filter(p => p.alive && p.alignment === 'EVIL' && p.role !== 'IMP');
  ...
  static scarletWomanPassive(game: GameState, playerId: string): void {
    ...
    if (player && !impPlayer?.alive && alivePlayers.length >= 5 && player.role === 'SCARLET_WOMAN') {
      player.role = 'IMP';
    }
  }
  ...
  static librarianSetup(game: GameState, playerId: string): PlayerInfo {
    ...
    const outsiderPlayers = game.players.filter(p => p.role && outsiders.includes(p.role));
  ...
  static fortuneTellerSetup(game: GameState, playerId: string): void {
    ...
    const goodPlayers = game.players.filter(p => p.alignment === 'GOOD' && p.id !== playerId);
  ...
  static fortuneTellerNightAction(game: GameState, playerId: string, targetIds: string[]): PlayerInfo {
    ...
    const targets = targetIds.map(id => game.players.find(p => p.id === id)).filter(Boolean) as Player[];
    ...
    return {
      ...
      information: {
        ...
        message: isDemonDetected ? 'YES - at least one is a Demon' : 'NO - neither is a Demon'
      },
      night_received: game.day_number
    };
  }

  static monkNightAction(game: GameState, playerId: string, targetId: string): NightAction {
    const target = game.players.find(p => p.id === targetId);
  ...
  static ravenkeeperNightAction(game: GameState, playerId: string, targetId: string): PlayerInfo {
    const target = game.players.find(p => p.id === targetId);
  ...
  static virginDayAction(game: GameState, playerId: string, nominatorId: string): { success: boolean; message: string } {
    ...
    const townsFolk: Role[] = ['WASHERWOMAN', 'LIBRARIAN', 'INVESTIGATOR', 'CHEF', 'EMPATH', 'FORTUNE_TELLER', 'UNDERTAKER', 'MONK', 'RAVENKEEPER', 'VIRGIN', 'SLAYER', 'SOLDIER', 'MAYOR'];
  ...
  static butlerNightAction(game: GameState, playerId: string, masterId: string): NightAction {
  ...
  static impNightActionEnhanced(game: GameState, playerId: string, targetId: string): NightAction {
    const target = game.players.find(p => p.id === targetId);
    const imp = game.players.find(p => p.id === playerId);
    ...
    if (target.role === 'MAYOR') {
      const otherPlayers = game.players.filter(p => p.alive && p.id !== targetId && p.id !== playerId);
