...
export type ActionType = 'INFORMATION' | 'TARGETED' | 'KILL' | 'PROTECT' | 'MODIFY';
...
export const ROLE_DEFINITIONS: Record<Role, RoleDefinition> = {
  ...
  SLAYER: {
    ...
    actions: [{
      ...
      timing: 'DAY',
      targets: 1,
      description: 'Once per game, publicly target a player - if Demon, they die'
    }],
  ...
  LIBRARIAN: {
    ...
    name: 'Librarian',
    alignment: 'GOOD',
    description: 'You start knowing that 1 of 2 players is a particular Outsider, or that none are.',
    actions: [{
      type: 'INFORMATION',
      timing: 'SETUP',
      targets: 0,
  ...
  FORTUNE_TELLER: {
    ...
    actions: [{
      ...
      timing: 'NIGHT',
      targets: 2,
      description: 'Check 2 players for Demon (may get false positive from red herring)'
    }],
  ...
  RAVENKEEPER: {
    ...
    name: 'Ravenkeeper',
    alignment: 'GOOD',
    description: 'If you die at night, you may choose a player: you learn their character.',
    actions: [{
      type: 'INFORMATION',
      timing: 'NIGHT',
      targets: 1,
  ...
  DRUNK: {
    ...
    name: 'Drunk',
    alignment: 'GOOD',
    description: 'You do not know you are the Drunk. You think you are a Townsfolk character, but your ability malfunctions.',
    actions: [{
      type: 'MODIFY',
      timing: 'PASSIVE',
      targets: 0,
...
export function getRolesByType(type: 'TOWNSFOLK' | 'OUTSIDER' | 'MINION' | 'DEMON'): Role[] {
  ...
  switch (type) {
    ...
    case 'OUTSIDER': return outsiders;
...
export function getRolesByTiming(timing: ActionTiming): Role[] {
  return Object.keys(ROLE_DEFINITIONS).filter(
    role => ROLE_DEFINITIONS[role as Role].actions.some(action => action.timing === timing)
  ) as Role[];
}

export function canRoleActAt(role: Role, timing: ActionTiming): boolean {
  return ROLE_DEFINITIONS[role].actions.some(action => action.timing === timing);
}